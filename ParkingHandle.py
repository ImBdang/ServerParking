...
class ParkingManagement(BaseSolution):
    ...
    def process(self, im0: np.ndarray, json_file, id_cam) -> SolutionResults:
        """
        ...
        for region in self.json:
            ...
            self.capnhat(rg_occupied, i, id_cam)
    ...
    def __call__(self, im0: np.ndarray, json_file, id_cam) -> SolutionResults:
        return self.process(im0, json_file, id_cam)
    
    def capnhat(self, status: bool, i: int, id_cam):
        with open(f"status/cam{id_cam}.json", "r") as f:
            data = json.load(f)

        data["items"][str(i)]["status"] = status  

        with open(f"status/cam{id_cam}.json", "w") as f:
